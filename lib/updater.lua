...
--- True once GITHUB_REPO has been filled in.
function Updater.isConfigured()
    return GITHUB_REPO ~= nil and GITHUB_REPO ~= ""
end

local function notConfigured()
    UIManager:show(InfoMessage:new{
        text = _("No update source is configured yet."),
        timeout = 3,
    })
end
...
-- bookcard.koplugin/ or bookcard.koplugin-<branch>/).
--
...
function Updater.getInstalledVersion()
    local meta_path = deps.PluginUtil.dir .. "_meta.lua"
...
function Updater.checkBackground(on_update_found)
    if not Updater.isConfigured() then return end
...
function Updater.check(on_success)
    if not Updater.isConfigured() then return notConfigured() end
...
function Updater.install(zip_url, old_version, new_version, on_success, error_label)
    ...
    UIManager:scheduleIn(0.1, function()
        ...
        local cache_dir = DataStorage:getSettingsDir() .. "/bookcard_update"
        ...
        local plugin_path = deps.PluginUtil.dir:gsub("/$", "")
